...
/**
 * Ported from Telegram's blur3 "Liquid Glass" package. The uniform names,
 * order and math below are copied verbatim from upstream so the shader
 * contract stays compatible. The AGSL source itself (res/raw/liquid_glass_shader.agsl)
 * wasn't part of the ported files and had to be written from scratch to match
 * this uniform contract - expect it to need visual tuning once you see it running.
 */
@RequiresApi(api = 33)
public class LiquidGlassEffect {
    ...
    public LiquidGlassEffect(Context context, RenderNode node) {
        this.node = node;
        final String code = Blur3Compat.readRawResource(context, R.raw.liquid_glass_shader);
