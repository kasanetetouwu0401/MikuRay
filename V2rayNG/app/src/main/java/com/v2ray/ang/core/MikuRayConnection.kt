 ...
 * only happens in onStartCommand()), so [IMikuRayService.isRunning] stays the source of
...
class MikuRayConnection : ServiceConnection {
    ...
    private val serviceCallback = object : IMikuRayServiceCallback.Stub() {
        override fun stateRunning() = runOnMain { callback?.stateRunning() }
        override fun stateNotRunning() = runOnMain { callback?.stateNotRunning() }
        override fun stateStartSuccess() = runOnMain { callback?.stateStartSuccess() }
