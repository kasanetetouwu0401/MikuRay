...
class MainActivity : HelperBaseActivity(),
    ...
    /**
     * Returns the inner MaterialCardView that wraps the TabLayout.
     * It is the second-level card inside layout_tab_wrapper that has clipToOutline=true.
     */
    private fun findInnerTabCard(): android.view.View? {
        return try {
            val wrapper  = binding.layoutTabWrapper      // LinearLayout
            val outerCard = wrapper.getChildAt(0)        // outer MaterialCardView (colorCard)
            if (outerCard is android.view.ViewGroup) outerCard.getChildAt(0) else null
        } catch (_: Exception) { null }
    }

    private fun applyLiquidGlassTab() {
        LiquidGlassTabController.applyState(
            this,
            tabCardView    = findInnerTabCard(),
    ...
    private fun setupLiquidGlassReceiver() {
        ...
        liquidGlassReceiver = object : android.content.BroadcastReceiver() {
            override fun onReceive(context: android.content.Context?, intent: android.content.Intent?) {
                if (intent?.action == AppConfig.BROADCAST_ACTION_LIQUID_GLASS_TAB_CHANGED) {
                    applyLiquidGlassTab()
                }
            }
        }
        val filter = android.content.IntentFilter(AppConfig.BROADCAST_ACTION_LIQUID_GLASS_TAB_CHANGED)
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.TIRAMISU) {
