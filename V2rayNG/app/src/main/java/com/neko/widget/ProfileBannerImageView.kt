...
class ProfileBannerImageView @JvmOverloads constructor(
    ...
    /**
     * Loads the saved profile banner. Decoded bitmaps are kept in a small
     * process-wide [bitmapCache] keyed by URI, so every instance of this view
     * (one is freshly inflated per bottom sheet, plus the one in MainActivity)
     * applies an already-decoded image synchronously via [setImageBitmap] instead
     * of waiting on Glide's async callback. That's what avoids the brief blank/flicker
     * frame that otherwise shows up every time a bottom sheet using this view is
     * reopened or the window regains focus.
     */
                ...
                Glide.with(this)
                    ...
                    .into(object : CustomTarget<Bitmap>() {
                        override fun onResourceReady(resource: Bitmap, transition: Transition<in Bitmap>?) {
                            ...
                            this@ProfileBannerImageView.tag = targetTag
                        }
                        ...
                        override fun onLoadFailed(errorDrawable: Drawable?) {
                            ...
                            this@ProfileBannerImageView.tag = null
                        }
                    })
            } else {
                loadDefault()
    ...
    companion object {
        /**
         * Process-wide cache of decoded profile-banner bitmaps, keyed by URI string.
         * Cleared whenever [AppConfig.BROADCAST_ACTION_PROFILE_BANNER_CHANGED] fires
         * (new image picked, shape changed, or banner removed) so stale bitmaps never
